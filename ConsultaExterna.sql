...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`usuarios` (
  ...
  `usu_contraseña` VARCHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`personal` (
  ...
  `per_num_telefono` VARCHAR(15) NULL,
  ...
  PRIMARY KEY (`per_id`, `usuarios_usu_id`),
  UNIQUE INDEX `per_num_personal_UNIQUE` (`per_num_personal` ASC),
  UNIQUE INDEX `pr_rfc_UNIQUE` (`pr_rfc` ASC),
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`medicamento` (
  ...
  `med_gramaje` FLOAT NULL,
  `med_nombre` VARCHAR(70) NULL,
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`recetas_medicamentos` (
  ...
  PRIMARY KEY (`recmed_folio`, `recmed_codigo`),
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`pacientes` (
  `pac_num_seguro` INT NOT NULL AUTO_INCREMENT,
  ...
  `pac_num_telefono` VARCHAR(15) NULL,
  PRIMARY KEY (`pac_num_seguro`),
  UNIQUE INDEX `pac_num_telefono_UNIQUE` (`pac_num_telefono` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`consultas` (
  ...
  PRIMARY KEY (`con_id`, `con_folio_receta`, `con_id_personal`, `con_seguro_paciente`),
  INDEX `fk_consultas_recetas1_idx` (`con_folio_receta` ASC),
  INDEX `fk_consultas_personal1_idx` (`con_id_personal` ASC),
  ...
  CONSTRAINT `fk_consultas_recetas1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_consultas_personal1`
    FOREIGN KEY (`con_id_personal`)
    REFERENCES `SCE_POSEC`.`personal` (`per_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`citas` (
  ...
  PRIMARY KEY (`cit_id`, `cit_id_personal`, `cit_num_seguro_paciente`),
  INDEX `fk_citas_personal1_idx` (`cit_id_personal` ASC),
  INDEX `fk_citas_pacientes1_idx` (`cit_num_seguro_paciente` ASC),
  CONSTRAINT `fk_citas_personal1`
    FOREIGN KEY (`cit_id_personal`)
    REFERENCES `SCE_POSEC`.`personal` (`per_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `SCE_POSEC`.`registros` (
  ...
  `personal_per_id` INT NOT NULL,
  `personal_usuarios_usu_id` INT NOT NULL,
  PRIMARY KEY (`reg_id`, `personal_per_id`, `personal_usuarios_usu_id`),
  INDEX `fk_registros_personal1_idx` (`personal_per_id` ASC, `personal_usuarios_usu_id` ASC),
  CONSTRAINT `fk_registros_personal1`
    FOREIGN KEY (`personal_per_id` , `personal_usuarios_usu_id`)
    REFERENCES `SCE_POSEC`.`personal` (`per_id` , `usuarios_usu_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
